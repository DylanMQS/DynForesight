...
def _read_video_uint8(path: Path) -> np.ndarray:
    ...
    # 1) imageio's FFMPEG plugin (most portable when ``imageio_ffmpeg`` is installed)
...
def process_one(
    ...
    *,
    ...
    if frame_indices is not None:
        bad = [i for i in frame_indices if i < 0 or i >= T]
        if bad:
            raise ValueError(
                f"--frame-indices contain out-of-range values {bad} for video with T={T} frames"
            )
        idxs = list(frame_indices)
    else:
...
def main():
    ...
    parser.add_argument("--frame-indices", type=str, default=None,
                        help="Optional comma-separated list of frame indices "
                             "(e.g. '0,4,8,16'). Overrides --num-frames when given.")
    ...
    if args.frame_indices is not None:
        try:
            ...
            raise SystemExit(f"Invalid --frame-indices {args.frame_indices!r}: {e}")
        if not frame_indices:
            raise SystemExit("--frame-indices parsed to empty list")
